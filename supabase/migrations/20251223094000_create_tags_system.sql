...
CREATE POLICY "Admins can create tags" ON public.tags FOR INSERT WITH CHECK (
    EXISTS (
...
CREATE POLICY "Anyone can view resource_tags" ON public.resource_tags FOR SELECT USING (true);

CREATE POLICY "Admins can create resource_tags" ON public.resource_tags FOR INSERT WITH CHECK (
    EXISTS (
        SELECT 1 FROM public.profiles
        WHERE id = auth.uid() AND role = 'admin'
    )
...
-- Insert sample tags
INSERT INTO public.tags (name, slug, display_order) VALUES
    ('Practice Tools', 'practice-tools', 1),
