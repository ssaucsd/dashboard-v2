...
export function RsvpListDialog({ eventId, eventTitle, trigger }: RsvpListDialogProps) {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'going': return 'bg-green-500/10 text-green-600 hover:bg-green-500/20';
            case 'maybe': return 'bg-yellow-500/10 text-yellow-600 hover:bg-yellow-500/20';
            case 'not_going': return 'bg-red-500/10 text-red-600 hover:bg-red-500/20';
                ...
                <DialogHeader>
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            ...
                            className="h-6 w-6"
                        >
                            <HugeiconsIcon icon={RefreshIcon} className={`h-3 w-3 ${isLoading ? 'animate-spin' : ''}`} />
                        </Button>
                    </div>
                                    ...
                                    return (
                                        <div key={status} className="space-y-3">
                                            ...
                                            <div className="space-y-3">
                                                {items.map((rsvp) => (
                                                    <div key={rsvp.user_id} className="flex items-center justify-between group">
                                                        <div className="flex items-center gap-3">
                                                            <Avatar className="h-9 w-9 border">
                                                                <AvatarFallback>
                                                                    {getInitials(rsvp.profile?.first_name, rsvp.profile?.last_name)}
                                                                </AvatarFallback>
                                                            </Avatar>
